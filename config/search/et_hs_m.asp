...
{
	form.action = "et_hs_m.asp";
	form.submit();
}
function goSelect(code, hs, charger)
{
	location.href="http://m.koreaart.ac.kr/lib/et_hs.asp?code="+escape(code)+"&hs="+escape(hs)+"&charger="+escape(charger);
}
	...
	<td align="center" bgcolor="#f8dfe0">
		...
		<td width="396" valign="top" bgcolor="#ffffff">
			...
			<td height="112" valign="top">
				...
				if (hs<>"") then
					oQry = ""
					oQry = "select HsCode, Name, Sido, regionCharger as Charger from dbo.TB_HighSchool where (Name like '%"&hs&"%') or (OldName1 like '%"&hs&"%') or (OldName2 like '%"&hs&"%')  or (OldName3 like '%"&hs&"%');"
					'Response.Write oQry
