...
{
	form.action = "et_hs_survey.asp";
	form.submit();
}
function goSelect(code, hs, charger)
{
	location.href="https://db.koreaart.ac.kr/et_hs.asp?code="+code+"&hs="+hs;
}
...
<form method="post">
	...
	<td align="center" bgcolor="#f8dfe0">
		...
		<td width="396" valign="top" bgcolor="#ffffff">
			...
			&nbsp;<span style="color:green;"> (문의 02-2063-0703)</span>
			</td>
			</tr>
			...
			<td height="112" valign="top">
				...
				if (hs<>"") then
					oQry = ""
					oQry = "select hsCode, hs , sido, charger from dbo.TB_HighSchool where (hs like '%"&hs&"%') or (oldHs like '%"&hs&"%');"
					'Response.Write oQry
					oRs.Open oQry,oConn,3,1
					if not ors.eof and not ors.bof then
						while not ors.eof
							%>
							<a href="javascript:goSelect('<%=oRs("hsCode")%>','<%=oRs("hs")%>', '<%=oRs("Charger")%>');" title="선택하세요!">(<%=oRs("sido")%>)&nbsp;<%=oRs("hs")%></a><br>
							<%
							oRs.Movenext
						wend
					else
